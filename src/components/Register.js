    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-xl shadow-gray-200/50 sm:rounded-lg sm:px-10">
          {error && (
            <div className={`mb-4 p-4 rounded-md ${showTermsError ? 'bg-yellow-50 border border-yellow-100' : 'bg-red-50 border border-red-100'}`}>
              <p className={`text-sm ${showTermsError ? 'text-yellow-700' : 'text-red-700'} text-center`}>{error}</p>
            </div>
          )}
